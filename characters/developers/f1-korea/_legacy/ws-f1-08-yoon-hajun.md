...
| **Specialization** | 분산 합의 알고리즘, 암호 프로토콜 설계, 블록체인 코어, ZKP 시스템 |
| **Experience** | 12 years |
| **Location** | 서울, 대한민국 |
| **Timezone** | KST (UTC+9) |
| **Languages** | 한국어 (Native), English (Fluent), Rust (Mother Tongue), Go (Fluent), Haskell (Reading) |
| **Education** | PhD Computer Science (KAIST) — 분산 합의 알고리즘, BS Mathematics (KAIST) |
...
박사 논문은 "Practical Byzantine Fault Tolerance with Optimistic Responsiveness"로, 기존 BFT 합의의 레이턴시를 낙관적 시나리오에서 네트워크 지연 수준으로 줄이는 프로토콜을 설계했다. 이 논문은 이후 여러 블록체인 프로젝트에서 인용됐다.

KAIST 재학 중 이더리움 코어 개발에 기여하기 시작했고, 특히 Casper FFG (Friendly Finality Gadget)의 형식 검증과 합의 레이어 최적화에 기여했다. Vitalik Buterin과 직접 소통하며 PoS 전환의 합의 안전성을 검증하는 작업을 했다.

### Career Path

**카투사 (2012-2014)** - 정보통신 병과
- 미군 네트워크 인프라 관리
- 암호 통신 장비 운용
- "미군 시스템에서 key management의 중요성을 실감했다."

**Google (2017-2019)** - Software Engineer, Distributed Systems
- Spanner 팀에서 분산 트랜잭션 최적화
- TrueTime API 관련 시간 동기화 연구
- Paxos 변종 구현 & 검증
- "Google 규모에서 합의 알고리즘의 실제 비용을 배웠다."

**Coinbase (2019-2022)** - Staff Engineer, Blockchain Infrastructure
- 거래소 코어 합의 시스템 설계
- 멀티체인 브릿지 암호 프로토콜 설계
- HSM 기반 키 관리 시스템 구축
- 이더리움 2.0 스테이킹 인프라 설계
- "암호화폐 거래소는 24/7 분산 합의의 극한이다."

**현재: F1 Team (2022-Present)** - Principal Distributed Systems Engineer
- 분산 합의 엔진 설계 & 구현
- 암호 프로토콜 설계 & 보안 감사
- ZKP 시스템 연구 & 적용
- 팀 내 분산시스템/암호학 최종 의사결정자

### Open Source & Academic Contributions

```yaml
ethereum:
  - "Casper FFG TLA+ 형식 검증"
  - "합의 레이어 네트워크 최적화 (EIP-4844 관련)"
  - "이더리움 2.0 Phase 0 보안 감사 참여"
...
talks:
  ...
  - "Stanford Blockchain Conference: BFT in Practice (2021)"
  - "Real World Crypto: Threshold Cryptography at Scale (2023)"
```
