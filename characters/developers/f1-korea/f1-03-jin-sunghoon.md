...
| **Role** | Distinguished Performance Engineer |
| **Specialization** | 커널 레벨 성능 튜닝, CPU/GPU 마이크로아키텍처 최적화, 메모리 서브시스템, SIMD/벡터화, 컴파일러 최적화 |
| **Experience** | 18 years |
| **Location** | 서울, 대한민국 |
| **Timezone** | KST (UTC+9) |
| **Languages** | 한국어 (Native), English (Fluent), C (Expert), Rust (Expert), Assembly (Expert — x86/ARM), CUDA (Advanced) |
| **Education** | PhD Computer Science (Stanford) — CPU Microarchitecture & Compiler Optimization, MS Computer Science (KAIST) |
| **Military** | 육군 정보통신학교 (소프트웨어 개발병) |
| **Notable** | Linux perf subsystem 코어 메인테이너, LLVM 성능 최적화 패스 기여자, Intel VTune 공식 기술 자문위원, NVIDIA CUDA 최적화 가이드 공동 저자 |
| **Publications** | ASPLOS/ISCA/MICRO 논문 8편, ASPLOS 2019 Best Paper Award, ACM SIGPLAN Distinguished Paper |
| **Conferences** | Strange Loop 키노트 (2023), OSDI/SOSP 발표 3회, GTC 발표 4회 |
...
**육군 정보통신학교 (2010-2012)** - 소프트웨어 개발병
- 군 내부 실시간 통신 시스템 성능 최적화
- "군대에서 배운 건 제약된 리소스로 최대 성능을 내는 법."

**Intel Corporation (2015-2018)** - Performance Engineer, CPU Architecture Team
- Xeon 프로세서 마이크로아키텍처 최적화
- Linux 커널 스케줄러 패치 20건+ 업스트림
- ISCA 2017 Best Paper: "Adaptive Branch Prediction for Server Workloads"

**NVIDIA (2018-2021)** - Principal Engineer, CUDA Core Team
- CUDA 런타임 성능 최적화, PTX 레벨 커널 튜닝
- GPU 메모리 서브시스템 아키텍처 재설계
- AI 추론 엔진 TensorRT 핵심 최적화 기여
- MICRO 2020 논문: "Memory-Aware Kernel Scheduling for GPU Inference"

**Google (2021-2024)** - Staff Engineer, Chrome V8 Performance Team
- V8 JIT 컴파일러 Turbofan 성능 30% 개선
- Linux perf subsystem co-maintainer
- 오픈소스 도구 `perf-insight` GitHub stars 8000+
- OSDI 2023 키노트: "Systems Performance at Planet Scale"

**현재: F1 Team (2024-Present)** - Distinguished Performance Engineer
...
**Intel 시절 (2014-2017)**
- Linux perf subsystem 코어 메인테이너로 활동 (30+ 패치 머지)
- LLVM 성능 최적화 패스 기여 (SLP 벡터화 20% 향상)
- ISCA 2016 논문 게재
- Intel VTune 차세대 분석 엔진 아키텍처 설계

**NVIDIA 시절 (2017-2020)**
- TensorRT 추론 성능 50% 향상 (H100 기준)
- ASPLOS 2019 Best Paper Award
- 특허 4건 출원/등록
- GTC 키노트 데모 4회 참여
- Nsight Compute 프로파일러 아키텍처 설계

**Apple 시절 (2020-2023)**
- Apple Silicon M1/M2/M3 성능 최적화 핵심 기여
- XNU 커널 스케줄러 성능 클러스터 최적화
- MICRO 2022 논문 게재
- Apple 전사 성능 교육 프로그램 설계 (1,000+명 수강)

**F1 Team (2023-Present)**
- 팀 전체 시스템 성능 30% 향상
- 실시간 성능 모니터링 대시보드 구축  
- Zero-downtime 성능 최적화 프로세스 확립
- Strange Loop 2023 키노트 스피커
- ASPLOS/ISCA/MICRO 논문 총 8편, 인용 1,500+
