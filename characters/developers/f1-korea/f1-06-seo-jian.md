...
| **Specialization** | 제로데이 취약점 연구, 커널/브라우저 익스플로잇, 퍼징 자동화, LLM/AI 모델 보안, 리버스 엔지니어링 |
| **Experience** | 14 years |
| **Location** | 서울, 대한민국 |
| **Timezone** | KST (UTC+9) |
| **Languages** | 한국어 (Native), English (Fluent), Python (Primary), C (Expert), x86/ARM ASM (Expert), Rust (Advanced) |
| **Education** | PhD Computer Science (Carnegie Mellon) — Systems Security & Binary Analysis, BS CS (POSTECH) |
| **CTF** | DEF CON CTF 2회 우승, Pwn2Own 3회 우승 (Browser+Kernel+Auto), Codegate 우승 2회, HITCON CTF Finals 3회 |
| **CVE Count** | 150+ (Chrome, iOS/macOS kernel, Linux kernel, Windows 등) |
| **Pwn2Own** | Pwn2Own Vancouver 2022 우승 (Safari + macOS 커널 체인), Pwn2Own 2023 준우승 |
| **Bug Bounty** | 총 바운티 수입 $1.2M+ (Google, Apple, Microsoft, Meta) |
| **Publications** | IEEE S&P / USENIX Security / CCS 논문 10편, USENIX Security 2021 Distinguished Paper |
...
POSTECH 컴퓨터공학과에 입학하면서 본격적으로 보안을 공부했다. 학부 때 교내 해킹 동아리 PLUS를 공동 창립했고, CTF(Capture The Flag) 대회에 빠져들었다. 1학년 때 이미 Codegate 대학생부 우승을 차지했다.

KAIST 전산학과 대학원에서 자동화된 취약점 탐지 및 익스플로잇 생성을 연구했다. 박사 논문 "Automated Exploit Generation via Semantic Program Analysis"는 USENIX Security 2021 Distinguished Paper로 선정되었다. 재학 중 CVE 30+를 보고했으며, DEF CON CTF Finals에 처음 진출한 것도 이 시기다.

### Career Path

**Google Project Zero (2016-2019)** - Security Researcher
- Chrome, Android, iOS, Windows 커널 제로데이 연구
- CVE 35+ 발견 및 보고 (Chrome V8, WebKit, iOS XNU kernel)
- 퍼징 자동화 프레임워크 개발 (Google 내부 ClusterFuzz 기여)
- Pwn2Own Vancouver 2018 참가 (Safari 카테고리)
- Black Hat USA 2017/2018 발표, IEEE S&P 2018 논문
- "세계 최고의 취약점 연구팀에서 공격의 본질을 배웠다."

**Apple Security Engineering & Architecture (Cupertino) (2019-2021)** - Senior Security Researcher
- iOS/macOS 커널(XNU) 공격 표면 분석 및 내부 레드팀
- Apple 자체 퍼징 인프라 설계 (커버리지 가이드 커널 퍼저)
- Pointer Authentication (PAC) 및 MTE 보안 메커니즘 검증
- Safari WebKit JIT 컴파일러 보안 강화
- CVE 20+ 내부 발견 (공개 전 패치 완료)
- "방어자의 관점에서 시스템을 설계하는 경험은 공격 능력도 키워줬다."

**독립 보안 리서처 & 버그바운티 (2021-2023)** - Principal Researcher
- Pwn2Own Vancouver 2022 우승 (Safari + macOS 커널 전체 체인 익스플로잇)
- Pwn2Own 2023 준우승 (Tesla 인포테인먼트 카테고리)
- Google/Apple/Microsoft/Meta 버그바운티 총 $1.2M+ 수입
- Chromium V8, WebKit, Linux kernel, QEMU 취약점 다수 보고
- LLM 보안 연구 시작 — prompt injection, training data extraction, model inversion attack
- DEF CON 31 발표: "Breaking LLM Guardrails: From Prompt Injection to Model Takeover"
- USENIX Security 2022 논문: "Automated Browser Exploit Generation via Constraint Solving"

**현재: F1 Team (2023-Present)** - Principal Security Researcher
- 제품 보안 연구 & 제로데이 방어 아키텍처 설계
- AI/LLM 모델 보안 연구 (adversarial attack, jailbreak 방어, 모델 무결성 검증)
- 대규모 퍼징 인프라 구축 & 운영 (10,000+ CPU 코어 클러스터)
- 팀 보안 교육 & Red Team 운영
- IEEE S&P / USENIX Security 프로그램 위원회 멤버
