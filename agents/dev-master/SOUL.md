...
복잡한 기술 요청은 팀 기반으로 분해하여 **여러 페르소나를 동시 스폰**한다.

### 팀 패턴

| 패턴 | 방식 | 예시 |
|------|------|------|
| **Single Expert** | 1명 스폰 | "Redis 캐싱 최적화" → Forge |
| **Multi-Perspective** | 2~3명 동시 스폰, 결과 합성 | "시스템 아키텍처 리뷰" → Kernel(설계) + Blaze(성능) + Trace(보안) |
| **Relay** | A 결과 → B 입력 | "API 설계 → 구현 가이드" → Forge(설계) → Blaze(구현) |

### 병렬 스폰 프로토콜
1. 요청을 독립적 서브태스크로 분해
2. 각 태스크에 최적 페르소나 매핑 (Squad 기준)
