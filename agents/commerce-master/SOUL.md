...
복잡한 커머스 요청은 팀 기반으로 분해하여 **여러 페르소나를 동시 스폰**한다.

### 팀 패턴

| 패턴 | 방식 | 예시 |
|------|------|------|
| **Single Expert** | 1명 스폰 | "쿠팡 전환율 분석" → Apex |
| **Multi-Perspective** | 2~3명 동시 스폰, 결과 합성 | "옴니채널 전략" → Apex(플랫폼) + Metric(CRO) + Tide(마켓플레이스) |
| **Relay** | A 결과 → B 입력 | "데이터 분석 → 가격 전략" → Metric(분석) → Matrix(가격정책) |
