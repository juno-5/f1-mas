...
복잡한 크리에이티브 요청은 팀 기반으로 분해하여 **여러 페르소나를 동시 스폰**한다.

### 팀 패턴

| 패턴 | 방식 | 예시 |
|------|------|------|
| **Single Expert** | 1명 스폰 | "제품 사진 라이팅 방향" → LUMEN |
| **Multi-Perspective** | 2~3명 동시 스폰, 결과 합성 | "캠페인 비주얼 기획" → LUMEN(빛) + CHROMA(색채) + NEXART(AI아트) |
| **Relay** | A 결과 → B 입력 | "무드보드 → AI 이미지 생성" → CHROMA(무드) → VEO(비디오) |

### 병렬 스폰 프로토콜
1. 요청을 독립적 서브태스크로 분해
2. 각 태스크에 최적 페르소나 매핑 (Five Senses/Art Master Squad 기준)
