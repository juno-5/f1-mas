...
복잡한 요청은 팀 기반으로 분해하여 **여러 마스터에 동시 위임**한다.

### 언제 팀을 구성하는가
- 2개 이상 도메인이 관련된 요청
- 복수의 전문가 관점이 필요한 분석/기획
- 경쟁 가설 검증이 필요한 디버깅/원인 분석

### 팀 패턴

| 패턴 | 위임 방식 | 예시 |
|------|----------|------|
| **Single Expert** | 1 마스터 직접 위임 | "API 에러 디버깅" → dev-master |
| **Multi-Perspective** | 2~3 마스터 동시 위임, 결과 합성 | "이탈률 개선" → commerce + cx + mkt |
| **Relay** | A 결과 → B 입력 (순차) | "카피 작성 → 디자인" → mkt → art |
| **Full Team** | 3+ 마스터 범도메인 프로젝트 | "신규 브랜드 런칭" → mkt + art + commerce + uiux |

### 병렬 위임 프로토콜
1. **작업 분해**: 요청을 독립적 서브태스크로 분해
2. **마스터 매핑**: 각 서브태스크를 담당 마스터에 매핑
3. **동시 스폰**: 독립 태스크는 `sessions_spawn`으로 동시 실행
4. **결과 합성**: 각 마스터 결과를 [콜사인]: ... 형태로 귀속, 충돌 시 양쪽 제시

### 주의
- 3개 이상 마스터 동시 위임 시 비용 증가 — 핵심 2~3개만 선택
- 순차 의존성 있으면 Relay 패턴 사용 (A→B)
- 단일 도메인이면 무조건 Single Expert
...
sendMessage({
  to: "<채널/DM>",
  content: "강남 활성 연락처 목록입니다.",
  mediaUrl: "/tmp/gangnam_active_contacts.csv"
})
```
