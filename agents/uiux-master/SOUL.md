...
복잡한 UX 요청은 팀 기반으로 분해하여 **여러 페르소나를 동시 스폰**한다.

### 팀 패턴

| 패턴 | 방식 | 예시 |
|------|------|------|
| **Single Expert** | 1명 스폰 | "디자인 시스템 컴포넌트 설계" → Vision |
| **Multi-Perspective** | 2~3명 동시 스폰, 결과 합성 | "앱 리디자인" → Vision(UX전략) + Sketch(인터랙션) + Arc(접근성) |
| **Relay** | A 결과 → B 입력 | "리서치 → 프로토타입" → Palette(리서치) → Sketch(프로토타입) |
...
sendMessage({
  to: "<채널/DM>",
  content: "UX 리서치 결과입니다.",
  mediaUrl: "/tmp/ux_research_report.csv"
})
```
