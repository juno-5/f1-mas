...
### 3.18 Higgsfield Soul 2.0 + Kling 3.0 UGC 워크플로우

Higgsfield가 **Soul 2.0**을 공식 출시. 패션, 문화, 크리에이티브 중심의 이미지 생성을 위해 처음부터 설계된 자체 이미지 모델. 단순 이미지 생성기가 아닌, 패션 레퍼런스·인터넷 문화·에디토리얼 맥락을 학습하여 "coquette", "Y2K 스트리트", "Portra 필름" 같은 표현을 정확히 이해하는 크리에이티브 디렉터급 모델.

**링크**: [Higgsfield](https://higgsfield.ai/)
...
| 모드 | 설명 |
|------|------|
| **SOUL** | 핵심 모델, 프롬프트 기반 이미지 생성 |
| **SOUL Reference** | 레퍼런스 이미지를 스타일 입력으로 활용 |
| **SOUL ID** | 일관된 캐릭터 생성 및 개인화 (사진 20장 업로드 기반) |

**2) 문화·패션에 대한 깊은 이해**
- 다양한 패션 시대, 컬렉션, 에스테틱, 인터넷 문화 기반 학습
- "Coquette"는 진짜 코켓처럼, 캠페인 비주얼은 실제 캠페인처럼 표현
- 에디토리얼도 코스튬처럼 과장되지 않고 자연스럽게 구현

**3) 카메라·매체 인식**
- iPhone, 일회용 카메라, 폴라로이드, Kodak Portra 등 프롬프트 반영
- 렌더링 느낌이 아닌, 실제 촬영된 것 같은 결과물 생성

**4) Nano Banana Pro를 위한 최고의 베이스 모델**
- 레퍼런스 이미지의 에스테틱을 이해하고 재해석 (복사가 아닌 재창조)
- Soul 2.0으로 생성 → Nano Banana Pro로 디테일 보정하는 워크플로우
- 상업용 이미지 생성·편집을 위한 최적의 조합

**5) 최고의 캐릭터 일관성**
- Soul ID + Soul 2.0은 가장 일관성 높은 개인화 이미지 모델
- 20장의 사진 기반으로 어떤 각도에서도 일관된 캐릭터 생성 가능
- 퍼스널 브랜딩, 캠페인, 스토리텔링에 최적

**6) 22개 이상의 크리에이티브 프리셋**
- Double Exposure · Warm Ambient · Retro BW · Y2K Street · Y2K Studio 등
- 프리셋은 필터가 아닌 크리에이티브 출발점으로 작동
- 자신의 프롬프트와 결합해 결과물을 더욱 확장 가능

#### 이미지 생성 워크플로우

**Step 1: Soul 2.0으로 리얼 UGC 이미지 생성**

Higgsfield → Soul 2.0 모델 선택 후 프롬프트 입력.

프롬프트 예시 (한국 여성 아이돌 분위기 UGC):
```
하얗고 투명한 피부의 20대 한국 여성 아이돌 분위기 모델, 자연스러운 생얼 메이크업과 은은한 홍조,
살짝 번진 듯한 얇은 아이라인과 촉촉한 립, 긴 블랙 헤어를 급하게 묶는 동작,
자기 방 책상 앞에 앉아 있는 구도, 프레임이 살짝 위에서 내려다보는 웹캠 시점,
작은 스탠드 조명 하나만 켜진 따뜻한 노란빛 밤 분위기, 창밖 어두운 도시 불빛과 유리창 반사,
어질러진 책상 위에 흩어진 메이크업 제품과 노트, 겹쳐 쌓인 책과 구겨진 메모지,
검정 슬리브리스 탑과 얇은 실버 목걸이, 채팅을 읽으며 웃다가 갑자기 정면을 응시하는 표정,
subtle skin texture, realistic pores, slight under-eye shadow, imperfect framing,
low contrast, soft haze in the air, slight lens fog effect, mild digital noise,
mixed warm and cool light tones, slightly soft focus, raw indoor lighting,
authentic Korean bedroom, too realistic UGC livestream mood, no visible camera equipment
```

변형 프롬프트 (브이로그 구도):
```
투명하고 하얀 피부의 20대 한국 여성 아이돌 분위기 모델, 청순한 얼굴과 자연스러운 핑크 블러셔,
맑은 눈빛과 촉촉한 입술, 긴 스트레이트 블랙 헤어,
자기 방 바닥에 앉아 벽에 기대고 스마트폰으로 셀프 촬영하는 브이로그 구도,
루즈한 홈웨어 티셔츠와 숏팬츠, 무릎을 끌어안은 편안한 자세,
방 안의 패브릭 러그와 작은 스탠드 조명이 보이는 생활 공간,
evening window light mixed with warm lamp light, slight motion blur on hand,
candid snapshot feeling, realistic skin texture, imperfect framing, raw digital noise,
too realistic, Korean idol influencer vibe, authentic UGC aesthetic
```

**Step 2: 캐릭터(Character) 기능으로 일관성 확보**

Higgsfield의 캐릭터 기능: 사진 약 20장 업로드 → 약 3~5분 학습 → 이후 무제한 일관된 캐릭터 이미지 생성. 얼굴 구조, 헤어, 표정 스타일까지 유지. 퍼스널 브랜딩·UGC 콘텐츠 제작에 최적.

**Step 3: Nano Banana Pro로 다양한 구도 생성**

Soul 2.0으로 생성한 이미지를 Nano Banana Pro에 레퍼런스로 입력하여 구도 변형:

```
1번 이미지의 여성분이 모든거 그대로 유지하고, 지금 카메라 방향으로 아이폰 핸드폰을
보여주는 모습 (라이브 스트리밍에 보여주는 모습)
아이폰 화면창엔 시간 밤 8시 화면
```

```
지금 화면 구도 모두 그대로 유지, 인물도 모두 유지
이 여자를 찍고 있는 카메라의 바로 윗쪽 (카메라 기준으로는 뒷쪽 벽이겠지)에 거울이 있는 것처럼 가정하고
그 거울을 바라보며 예쁜 흰색 상의 자켓 또는 가디건을 입으며, 옷 메무새를 잡는 모습
```

#### 영상 생성 워크플로우

**Step 1: Kling 3.0으로 영상 생성**

생성한 이미지를 Kling 3.0의 Start Frame / End Frame에 첨부.

**Step 2: 프롬프트 작성 (라이브스트림 UGC 예시)**

```
realistic livestream clip, Korean woman in her 20s sitting at a desk at night,
warm desk lamp lighting, subtle indoor haze, city lights through the window,
slightly top-down webcam angle, messy desk with books and makeup items,
authentic bedroom livestream atmosphere,

a mirror is positioned above the camera on the wall,
she looks slightly above the lens as if checking her reflection in that mirror,
she slowly buttons and adjusts a soft white cardigan over a black top,
minimal and natural movement, relaxed posture, calm breathing,
no dramatic expression change,

she speaks softly and calmly in Korean:
"근데 이거… 말씀드려도 돼요?"
(short natural pause)
"저 사실..하프던이 만든 사람이에요." (고백하듯이)

steady tone, neutral expression, eyes never directly locking into the lens,
subtle fabric movement, natural mouth sync, slight ambient room noise feeling,
imperfect exposure, mild digital noise, unstaged authentic livestream vibe
```

**파이프라인 요약**:
```
Soul 2.0 (기본 이미지) → Nano Banana Pro (구도 변형) → Kling 3.0 (Start/End Frame → 영상 생성)
```
...
19. [Higgsfield Soul 2.0](https://higgsfield.ai/) — Soul 2.0 + Nano Banana Pro + Kling 3.0 UGC 워크플로우
